  ...
  try {
    if (req.query.hasOwnProperty("id")) {
      ...
      if (supp[0] === 0) {
        ...
        const pay = vendor.filter(
          (item) => item.payment_status !== "FULL-PAID"
        ).length;
        if (pay === 0) {
          const ress = await tripModel.update(
            { paymentStatus: "FULL-PAID" },
            { where: { tripId: suppData.tripId } }
          );
      ...
      const supp = await vendorPayModel.create({
        vendor_pay_id: uuidv4(),
        ...suppData,
      });
      if (!supp) {
        res.status(400);
...
export const deleteSuppPay = asyncHandler(async (req, res) => {
  ...
  console.log(id, "id");
  try {
    const suppData = await vendorPayModel.findOne({
      where: {
        vendor_pay_id: id,
      },
    });
